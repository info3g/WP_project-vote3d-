...
class ItemLikes {
	...
	function like_this($post_id, $zero_postfix = false, $one_postfix = false, $more_postfix = false, $action = 'get') 
	{
		...
		$cus_user_id = get_current_user_id();
		global $wpdb;
		$check_value = array();
				
		$user_meta = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}usermeta` WHERE user_id=$cus_user_id and meta_key='post_likes'");
		$check_value = $user_meta[0]->meta_value; 
		$meta_key = "post_likes";
		$newString = "";
		$key = array_search($cus_user_id, $check_value);
		...
		switch($action) {
			...
			case 'update':
				...
				if(count($check_value)>0  ){
						$wpdb->query("UPDATE {$wpdb->prefix}usermeta SET meta_value=CONCAT(meta_value,',$post_id ') WHERE user_id='$cus_user_id'AND  meta_key='$meta_key'");
								
							} else{
								
								 $wpdb->query("INSERT INTO {$wpdb->prefix}usermeta (user_id,meta_key,meta_value) VALUES('$cus_user_id','$meta_key','$post_id')"); 
							}
				}
				if( $likes == 0 ) { $postfix = $zero_postfix; }
				elseif( $likes == 1 ) { $postfix = $one_postfix; }
	...
	function do_likes( $post_id = null )
	{
		...
		$check_value = $user_meta[0]->meta_value; 
		$meta_key = "post_likes";
		$lkey = array_search($cus_user_id, $check_value); 
		
  		$class = 'item-likes';
  		$title = sq_option('like_this_text', 'Like this');
		if( (isset($_COOKIE['item_likes_'. $post_id])) && isset($lkey) ){
