...
$act_id=$activities_template->activity->id;
 "SELECT privacy FROM `{$wpdb->prefix}rt_rtm_media` WHERE activity_id=$act_id"; 
$act_data = $wpdb->get_results("SELECT privacy FROM `{$wpdb->prefix}rt_rtm_media` WHERE activity_id=$act_id");
$chck_privacy = $act_data[0]->privacy;
